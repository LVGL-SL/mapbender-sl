	...
	function __construct() {
        ...
        */
	    ...
	    foreach ($originalJsonArray as $layerRecord) {
		    ...
		    if (!isset($returnObject->layer[$layerCount])) {
			$returnObject->layer[$layerCount] = new stdClass();
		    }
			...
			if (!in_array($metadataRecord['metadataId'], $metadataIdArray) && $metadataRecord['$metadataId'] !== 0) {
			    if (!isset($returnObject->layer[$layerCount]->metadata[$metadataCount])) {
				$returnObject->layer[$layerCount]->metadata[$metadataCount] = new stdClass();
			    }
				...
				foreach ($reducedMetadataArray as $featuretypeRecord) {
					...
					if (!isset($returnObject->layer[$layerCount]->metadata[$metadataCount]->featuretype[$featuretypeCount])) {
					    $returnObject->layer[$layerCount]->metadata[$metadataCount]->featuretype[$featuretypeCount] = new stdClass();
					}
					...
					if ($reducedFeaturetypeArray !== null) {
					    foreach ($reducedFeaturetypeArray as $wfsConfRecord) {
						    ...
						    if (!isset($returnObject->layer[$layerCount]->metadata[$metadataCount]->featuretype[$featuretypeCount]->wfsConf[$wfsConfCount])) {
							$returnObject->layer[$layerCount]->metadata[$metadataCount]->featuretype[$featuretypeCount]->wfsConf[$wfsConfCount] = new stdClass();
						    }
...
//		$e = new mb_notice("before: " . implode(", ", $wmsArray));
	      ...
	      if($this->wms_version == "1.0.0"){
			...
			if($section == "layer"){
				...
				if(mb_strtoupper($element['tag']) == "IDENTIFIER"){
					if (!isset($this->objLayer[$cnt_layer]->layer_identifier[$cnt_identifier])) {
						$this->objLayer[$cnt_layer]->layer_identifier[$cnt_identifier] = new stdClass();
					}
				...
				if($dataurl && mb_strtoupper($element['tag']) == "FORMAT"){
					if (!isset($this->objLayer[$cnt_layer]->layer_dataurl[$cnt_dataurl])) {
						$this->objLayer[$cnt_layer]->layer_dataurl[$cnt_dataurl] = new stdClass();
					}
				...
				if(mb_strtoupper($element['tag']) == "METADATAURL" && $element['type'] == "open"){
					$metadataurl = true;
					$cnt_metadataurl++;
					if (!isset($this->objLayer[$cnt_layer]->layer_metadataurl[$cnt_metadataurl])) {
						$this->objLayer[$cnt_layer]->layer_metadataurl[$cnt_metadataurl] = new stdClass();
					}
				...
				if(mb_strtoupper($element['tag']) == "DIMENSION"){
					...
					if (!isset($this->objLayer[$cnt_layer]->layer_dimension[$cnt_dimension])) {
						$this->objLayer[$cnt_layer]->layer_dimension[$cnt_dimension] = new stdClass();
					}
				...
				if ($this->wms_version == '1.1.1') {
					...
					if(mb_strtoupper($element['tag']) == "EXTENT"){
						...
						//extract 
						if (!isset($this->objLayer[$cnt_layer]->layer_dimension[$posDimensionName])) {
							$this->objLayer[$cnt_layer]->layer_dimension[$posDimensionName] = new stdClass();
						}
	...
	function newLayer ($currentLayer, $currentExtent) {
		...
		if (is_string($currentLayer["layer_dataurl"])) {
			if (!isset($newLayer->layer_dataurl[0])) {
				$newLayer->layer_dataurl[0] = new stdClass();
			}
		...
		if (is_string($currentLayer["layer_metadataurl"])) {
			if (!isset($newLayer->layer_metadataurl[0])) {
				$newLayer->layer_metadataurl[0] = new stdClass();
			}
...
//$e = new mb_exception("show coupling!");
			...
			while($row2 = db_fetch_array($res_layer)){
				...
				while($row3 = db_fetch_array($res_dataUrl)){
					if (!isset($this->objLayer[$layer_cnt]->layer_dataurl[$count_dataUrl])) {
						$this->objLayer[$layer_cnt]->layer_dataurl[$count_dataUrl] = new stdClass();
					}
				...
				if (defined("SHOW_INSPIRE_DOWNLOAD_IN_TREE") && SHOW_INSPIRE_DOWNLOAD_IN_TREE == true && $row2["downloadoptions"] != "") {
					...
					if (!isset($this->objLayer[$layer_cnt]->layer_dataurl[0])) {
						$this->objLayer[$layer_cnt]->layer_dataurl[0] = new stdClass();
					}
				...
				while($row3 = db_fetch_array($res_metadataUrl)){
					if (!isset($this->objLayer[$layer_cnt]->layer_metadataurl[$count_metadataUrl])) {
						$this->objLayer[$layer_cnt]->layer_metadataurl[$count_metadataUrl] = new stdClass();
					}
