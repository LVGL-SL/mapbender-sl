		...
		for ($i=0; $i < count($sqlTable['uuid']); $i++){
			...
			if (!$registratingDepartments || ($registratingDepartments != null && in_array($groupOwnerArray[4][$i],explode(',',$registratingDepartments)))) {
				if ($sqlTable['uuid'][$i] != $currentUuid) {
					...
					$inspireMonitoring['datasets'][$metadataIndex]->relevantArea = (string)ceil((double)$sqlTable['inspire_whole_area'][$i]);
					$inspireMonitoring['datasets'][$metadataIndex]->actualArea = (string)ceil((double)$sqlTable['inspire_actual_coverage'][$i]);
