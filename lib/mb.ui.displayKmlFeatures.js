...
var displayFeatures = {
    ...
    zoomToLayer: function(url, buffer = 0.015) {
    ...
    getBbox: function(feature) {
        function _getBbox(geometry) {
            switch (geometry.type.toLowerCase()) {
                case 'point':
                    var minx = geometry.coordinates[0] - 0.001;
                    var miny = geometry.coordinates[1] - 0.001;
                ...
                case 'geometrycollection':
                    var bboxes = geometry.geometries.map(function (g) {
                        return _getBbox(g);
                    });

                    return bboxes.reduce(function (prev, next) {
                        return [
        ...
        return _getBbox(feature.geometry);
    },
    ...
    renderPolygon: function(canvas, feature) {
        ...
        //Ticket #8549: Added logic to handle polygons with holes**
        //A polygon can consist of multiple rings, where the first ring is the outer ring
        //and the rest are holes. The coordinates are in the form:
        //feature.geometry.coordinates: [ [ [x,y], ... ], [ [x,y], ... ], ... ]
        //Each ring: first is outer, rest are holes
        $.each(feature.geometry.coordinates, function(_, ring) {
            var path = '';
            $.each(ring, function(i, v) {
                var pt = { x: v[0], y: v[1] };
                if (!feature.preview) {
                    pt = map.convertRealToPixel(pt);
                }
        ...
        // ** Draw all rings as one path, use evenodd fill rule for holes (Raphael)
        var svgPath = canvas.path(paths.join(''));
        svgPath.attr(feature.properties);
        if (svgPath.node) {
            svgPath.node.setAttribute('fill-rule', 'evenodd');
        }
    },
